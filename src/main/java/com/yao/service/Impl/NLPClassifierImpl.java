...
public class NLPClassifierImpl implements INLPClassifier {
    ...
    public String train(String userid, String robotid, String corpus, int train_flag) throws IOException {
        ...
        story.setTrain_time(now);
        storyDao.add(story);

        String rulename = userid + "_" + robotid + "_" + DateUtil.DataToString(now,"yyyyMMddHHmmSS");
        Rule rule = new Rule();
        rule.setCorpus(corpus);
        rule.setUserid(userid);
        rule.setRobotid(robotid);
        rule.setRulename(rulename);
        rule.setInsert_time(now);
        ruleDao.add(rule);
    ...
    public String predict(String userid, String robotid, int trian_flag, String content){
        String result = null;
        Story story = storyDao.getByUserAndRobotAndTrainFlag(userid, robotid,trian_flag).get(0);
        String model_path = story.getModel_path();
        //Bayes
